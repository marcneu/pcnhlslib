...
int main()
{
    ...
    array<T,INPUT_WIDTH_1+INPUT_WIDTH_2+INPUT_WIDTH_3> golden = {5, -2, -10, 0, 1, 2};

    hls::stream<array<T,INPUT_WIDTH_1+INPUT_WIDTH_2+INPUT_WIDTH_3>> outputStream;

    dut(aInputStream, bInputStream, cInputStream, outputStream);

    for(int ii = 0; ii < II; ii++) {
        {
            array<T,INPUT_WIDTH_1+INPUT_WIDTH_2+INPUT_WIDTH_3> output;
            outputStream >> output;
            for(int i = 0; i < INPUT_WIDTH_1+INPUT_WIDTH_2+INPUT_WIDTH_3; i++) {
